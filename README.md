# Wassim Tajeddin  
### Software Engineer | Java · Fullstack · AI/ML  
📍 Malmö, Sweden | 📧 wassim.tajeddin@gmail.com | 🔗 [LinkedIn](https://www.linkedin.com/in/wassim-tajeddin-82a563201/) | 🔗 [GitHub](https://github.com/wassimtajeddin) | 🌐 [Portfolio](https://wassimtajeddin.com/)  

---

## 🚀 Skills  
**Backend:** Java (Spring Boot), Python  
**Frontend:** Vue.js, JavaScript, HTML/CSS  
**Databases:** MySQL, MongoDB, SQLite  
**AI/ML:** TensorFlow, CNNs (98% MNIST accuracy)  
**DevOps:** Docker, Azure, CI/CD  
**Other:** Agile (Scrum), TDD  

---

## 🎓 Education  
**Java Developer** – *IT-högskolan, Gothenburg* | Aug 2022 – May 2024  
- 500+ hrs Java; built fullstack Spring Boot + Vue.js app.  

**Python for AI** – *IT-högskolan, Gothenburg* | Jan 2025 - Mar 2025
- Developed CNN digit recognizer (98% accuracy).  

**Physics** – *Aleppo University* | Sep 2011 – Sep 2015  

---

## 💻 Projects  
**Enterprise App** – Spring Boot 3, JPA/Hibernate, MySQL, CI/CD · [GitHub](https://github.com/wassimtajeddin/springboot)  
**QuizHub** – Agile team quiz app, Vue.js + Node.js, CI/CD · [GitHub](https://github.com/wassimtajeddin/AgilProjekt)  
**MNIST Classifier** – CNN model in TensorFlow (98% accuracy) · [GitHub](https://github.com/wassimtajeddin/SVM-MNIST-Handwritten-Digit-Classifier)  

---

## 💼 Experience  
**Android Developer Intern** – *Junico AB* | Nov 2023 – Mars 2024  
- Built Jetpack Compose + Kotlin app with Firebase DB in Scrum team.  

**Team Leader** – *PostNord AB* | June 2018 – Feb 2024  
- Managed teams and workflows, ensuring delivery & customer satisfaction.  

---

## 🌐 Languages  
Swedish (Fluent) · English (Fluent) · Arabic (Native)  

---

📄 [Resume PDF](https://github.com/wassimtajeddin/wassimtajeddin/blob/main/Wassim_Tajeddin_-_Software_Developer.pdf)
